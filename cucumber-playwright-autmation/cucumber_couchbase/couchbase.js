...
app.post("/store", async (req, res) => {
    ...
    try {
        ...
        // Read the entire report data as a string
        const reportData = fs.readFileSync(filePath, "utf-8");

        // Parse the report data into a JavaScript object
        const parsedData = JSON.parse(reportData);

        // Generate a new unique ID using Date.now() for each request
        const newId = `report-${Date.now()}`;  // Use Date.now() for a unique ID (you can add a prefix like 'report-')

        // Use the entire report data as the object to store in Couchbase with the new unique ID
        const rustBookData = {
            id: newId,  // Use the newly generated unique ID
            report: parsedData,  // Store the entire parsed report data
            createdAt: new Date(),
        };

        // Store the entire report object in Couchbase with the new ID
...
// Example endpoint to retrieve stored data (optional)
